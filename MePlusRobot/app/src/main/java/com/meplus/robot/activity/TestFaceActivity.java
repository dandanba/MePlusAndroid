...
public class TestFaceActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        final Handler logHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                mLogText.append(msg.obj.toString());
                mLogText.append("\n-------------\n");
            }
        };
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    logText(logHandler, result.toString());
                    System.out.println(message = result.getJSONArray("face").getJSONObject(0).getJSONObject("position").getJSONObject("center").toString());
                    logText(logHandler, message);
                    //-----------------Person-----------------
                    //person/create
                    System.out.println("\nperson/create");
                    ...
                    System.out.println("\nperson/set_info");
                    for (int i = 0; i < result.getJSONArray("face").length(); ++i) {
                        new PostParameters().setPersonName("person_" + i).setTag("���� tag_" + i).getMultiPart().writeTo(System.out);
                    ...
                    System.out.println("\nperson/get_info");
                    for (int i = 0; i < result.getJSONArray("face").length(); ++i)
                        //                    error :com.facepp.error.FaceppParseException: API error. code=1503, message=NAME_EXIST, responseCode=453 TODO 需要检查 453 错误
                    ...
                    System.out.println("\nfaceset/set_info");
                    for (int i = 0; i < result.getJSONArray("face").length(); ++i) {
                        new PostParameters().setFacesetName("faceset_" + i).setTag("���� tag_" + i).getMultiPart().writeTo(System.out);
                        System.out.println(httpRequests.facesetSetInfo(new PostParameters().setFacesetName("faceset_" + i).setTag("���� tag_" + i)));
                    }
    ...
    public void logText(Handler logHandler, String result) {
        final Message msg = logHandler.obtainMessage();
        msg.obj = result;
